...
const run = async () => {
  ...
  for (const t of TARGETS) {
    ...
    /*
     * Shoot with the AMBER accent, which is the shipped opt-in
     * ([data-accent='matte']), not a capture-only hack. The store art
     * composes these shots under amber captions and the brand mark's amber
     * tile, and the brand rule is one accent hue per composition — a green
     * in-game accent inside an amber-captioned frame is exactly the mix the
     * rule exists to prevent.
     */
    await page.evaluateOnNewDocument(() => {
      try {
        localStorage.setItem('ngw-wordy/accent', 'matte');
      } catch {}
    });

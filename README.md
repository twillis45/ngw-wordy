...
ring with one lit in steel-blue, i.e. the game itself. The social card is
`scripts/build-og.py` (`public/og.png`, 1200x630) — a static export has no
runtime, so `next/og` cannot generate one per request. Both scripts are run by
hand and their output is committed.

### Search and social

`NEXT_PUBLIC_SHARE_URL` is the deployed origin, and now feeds four more things
through [`src/lib/site.ts`](src/lib/site.ts): `metadataBase`, the canonical
link on every page, `og:url`, and the absolute URLs in `sitemap.xml` /
`robots.txt`. It defaults to `http://localhost:3000` on purpose — a guessed
production domain would ship a sitemap pointing at a host nobody owns.

`src/app/sitemap.ts` lists the four routes by hand; a new route means a new
line there.

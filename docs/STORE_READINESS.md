...
| 5.4 | **React hydration mismatch on every page load** | **TODO** | Found 2026-08-14 while verifying an unrelated lint fix. The production export throws `Minified React error #418` — server-rendered HTML did not match the client — on a plain load with no share link, at 390x844. Measured on `main` at 8291784 as well as on the branch, so nothing recent caused it. One error, the page recovers, and it is invisible to a player, which is exactly why it has survived: React discards the server HTML and re-renders that subtree on the client, so the prerender is being paid for and thrown away. Worth finding before a store build, since a wrapper makes first paint more visible, not less. Repro: `npm run build`, serve `out/` and read the console. |
